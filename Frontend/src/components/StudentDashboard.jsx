...
const SearchEventsPage = ({ onEventClick, searchTerm }) => {
...
const MessagesPage = ({ onMentorClick }) => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-xl h-[calc(100vh-220px)] flex">
      {" "}
              ...
              >
                ...
                <div
                  className={`p-3 rounded-lg max-w-md shadow-sm ${
                    msg.sender === "me" ? "bg-cyan-500 text-white" : "bg-white"
                  }`}
                >
                  {" "}
                  {msg.text}{" "}
...
// --- Support Page (FIXED & FULLY FUNCTIONAL) ---
const SupportPage = ({ user }) => {
  ...
  const handleSend = async (messageText = input) => {
    ...
    // Handle predefined questions first
    ...
    // --- Logic for custom questions (AI API Call) ---
    const GEMINI_API_KEY = import.meta.env.VITE_GEMINI_API_KEY;

    // If no API key, provide a fallback response
    if (!GEMINI_API_KEY) {
      setTimeout(() => {
    ...
    // If API key exists, make the API call
